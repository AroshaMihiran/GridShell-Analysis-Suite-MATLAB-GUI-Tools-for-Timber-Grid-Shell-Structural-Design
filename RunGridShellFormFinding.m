function RunGridShellFormFinding(csv_dir)
% RunGridShellFormFinding - Form Finding Only Analysis (No Member Sizing)
% Input: csv_dir - directory containing CSV files from GUI

    clc;
    fprintf('=== GridShell Form Finding Analysis Starting ===\n');
    ...
    FontSizes.Title = 18;
    FontSizes.AxisLabel = 16;
    FontSizes.TickLabel = 14;
    FontSizes.Legend = 14;
    FontSizes.ColorbarLabel = 16;
        ...
        for i = 1:row_count
            try
                ...
                if ~isempty(string_parts)
                    % Remove '[' from first string and ']' from last string
        ...
        while ~feof(fid)
            ...
            if ischar(line) && ~isempty(strtrim(line))
                ...
                if length(parts) >= 2
                    ...
                    panel_loads(load_row_count) = 1; %#ok<AGROW>
                end
            end
        end
        fclose(fid);
    ...
    if exist(load_pattern_file, 'file')
        ...
        q1_panels = zeros(max(Ne,1), 1);
        q2_panels = zeros(max(Ne,1), 1);
    ...
    %% LOAD MEMBER MATERIALS AND AREAS FROM CSV (MODIFIED)
    material_file = fullfile(csv_dir, 'member_materials.csv');
    if exist(material_file, 'file')
        fprintf('Loading member materials and areas from CSV...\n');
        ...
        % NEW: Load Areas from CSV
        if ismember('Area', material_data.Properties.VariableNames)
            A = material_data.Area;  % Areas in m²
            fprintf('  Areas loaded from CSV: %d values\n', length(A));
        else
            A = 0.005 * ones(Nb, 1);  % Default areas
            fprintf('  Warning: No Area column found, using default areas (0.005 m²)\n');
        end
    else
        % Default values if not provided
        fprintf('Using default materials and areas...\n');
    ...
    A_cm2 = A * 10000;  % Convert to cm² for display
    ...
    fprintf('  Area range: %.4f - %.4f m² (%.1f - %.1f cm²)\n', min(A), max(A), min(A_cm2), max(A_cm2));
    
    %% LOAD FORM FINDING PARAMETERS FROM CSV
    param_file = fullfile(csv_dir, 'analysis_parameters.csv');
    if exist(param_file, 'file')
        fprintf('Loading form finding parameters from CSV...\n');
        ...
        fprintf('Form finding parameters file not found, using defaults...\n');
        ff_tol = 1e-4;
        ff_max_iter = 50;
    end
    
    %% FORM FINDING ONLY (NO MEMBER SIZING)
    fprintf('\nStarting Form Finding Analysis...\n');
    tic; % <-- ADD THIS LINE to start the timer
    ...
    % Get free edge indices
    free_edge_indices = setdiff(1:Nb, fixed_edge_indices);
    
    % Calculate EA values using given areas
    EA_values = E005_members .* A;
    ...
    x(3:3:Nf) = 0;  % Set Z displacements to 0 initially
    x0 = x;
    U(FREE) = x;
    DNODE = NODE_ori + reshape(U,3,Nn)';
    
    % Initialize PE tracking for FF
    PE_FF_history = [];
    PE_detailed_FF = [];
    
    % Calculate loads - HANDLE EMPTY ELEMENT CASE
    opts = struct();
    if Ne > 0
    ...
    % Form finding iteration with enhanced tracking
    tol = ff_tol;
    iter_limit = ff_max_iter;
    ff_iter = 0;
    res1 = 1;
    FF_convergence_history = [];
    fprintf('Running form finding with PE and gradient tracking...\n');
    
    % Clear PE tracking data before optimization
    PE_iteration_data = struct('iteration_count', 0, 'values', [], 'iterations', [], ...
                              'gradient_norms', [], 'max_gradient_components', []);
    
    % First form finding optimization with detailed tracking
    [x,PE,~,output] = fminunc(@(Uf)GetPotential(NODE_ori,BARS,Uf,F0,K,FREE,L0),x,options_FF_enhanced);
    
    % Store the detailed tracking for FF optimization
    PE_detailed_FF = PE_iteration_data;
    ...
    % Continue form finding iterations if needed
    while res1 > tol && ff_iter < iter_limit
        ...
        FF_convergence_history = [FF_convergence_history; res1];
        x0 = x;
        fprintf('  FF Iter %d: PE = %.6e, Residual = %.6f\n', ff_iter, PE, res1);
    end
    
    if ff_iter >= iter_limit
        fprintf('  Warning: Form finding reached maximum iterations (%d)\n', iter_limit);
    else
        fprintf('  Form finding converged after %d iterations\n', ff_iter);
    end
    analysis_duration = toc; % <-- ADD THIS LINE to stop the timer and store the time
    fprintf('<strong>Analysis completed in %.2f seconds (%.2f minutes).</strong>\n', analysis_duration, analysis_duration/60); % <-- ADD THIS LINE
    if ~isempty(FF_convergence_history)
        figure('Name','FF Convergence Criteria','Position',[150 150 800 600]);
        semilogy(0:length(FF_convergence_history)-1, FF_convergence_history, 'b-o', 'LineWidth', 2, 'MarkerSize', 6);
        hold on;
        yline(ff_tol, 'r--', sprintf('Tolerance = %.0e', ff_tol), 'LineWidth', 2);
        xlabel('FF Iteration', 'FontSize', FontSizes.AxisLabel);
        ylabel('||X^{(k)} - X^{(k-1)}|| / N_{dof} (log scale)', 'FontSize', FontSizes.AxisLabel);
        title('Form Finding Convergence Criteria', 'FontSize', FontSizes.Title, 'FontWeight', 'bold');
        grid on;
        legend({'Convergence Metric', 'Tolerance'}, 'FontSize', FontSizes.Legend);
    end
    % Update lengths to final configuration
    L_final = GetL(NODE, BARS);
    %% FINAL CALCULATIONS WITH GIVEN AREAS
    fprintf('\nFinal calculations with given areas...\n');
    
    % Calculate final stresses using given areas
    [S_final, ~, ~] = GetSigma(NODE, BARS, E005_members, L0);
    F_member = S_final .* A;  % Force = Stress × Area
    
    % Calculate stress ratios using given areas
    iz = sqrt(A) ./ (2*sqrt(3));
    lmz = L0 ./ iz;
    ...
    fprintf('  Min stress ratio: %.3f\n', min(stress_ratio));
    fprintf('  Mean stress ratio: %.3f\n', mean(stress_ratio));
    
    %% ENHANCED PLOTTING SECTION (FORM FINDING ONLY)
    fprintf('Generating comprehensive plots...\n');
    
    % Generate form finding specific plots
    generateFormFindingPlots(NODE, BARS, FIX_NODE, FREE_NODE, A_cm2, stress_ratio, stress_values, S_final, F_member, ...
                           fixed_edge_indices, free_edge_indices, FontSizes, PE_detailed_FF, PE_FF_history, ...
                           Nn, Nb, L0, A);
    ...
    % Save summary
    summary_file = fullfile(csv_dir, 'form_finding_summary.txt');
    fid = fopen(summary_file, 'w');
    fprintf(fid, 'GRIDSHELL FORM FINDING ANALYSIS SUMMARY\n');
    fprintf(fid, '=====================================\n\n');
    ...
    fprintf(fid, '\nForm Finding:\n');
    fprintf(fid, '  Analysis Duration: %.2f seconds\n', analysis_duration);
    fprintf(fid, '  FF Iterations: %d\n', ff_iter);
    fprintf(fid, '  FF Tolerance: %.2e\n', ff_tol);
    fprintf(fid, '  Final Residual: %.6f\n', res1);
    fprintf(fid, '  Final PE: %.6e\n', PE);
    fprintf(fid, '\nResults (using given areas):\n');
    fprintf(fid, '  Max Area: %.4f m² (%.2f cm²)\n', max(A), max(A_cm2));
    fprintf(fid, '  Min Area: %.4f m² (%.2f cm²)\n', min(A), min(A_cm2));
    fprintf(fid, '  Mean Area: %.4f m² (%.2f cm²)\n', mean(A), mean(A_cm2));
    fprintf(fid, '  Max Stress Ratio: %.3f\n', max(stress_ratio));
    fprintf(fid, '  Min Stress Ratio: %.3f\n', min(stress_ratio));
    fprintf(fid, '  Mean Stress Ratio: %.3f\n', mean(stress_ratio));
    fprintf(fid, '  Total Volume: %.6f m³\n', sum(A .* L_final));
    fclose(fid);
    
    fprintf('\n=== FORM FINDING ANALYSIS COMPLETE ===\n');
    fprintf('Results saved to: %s\n', csv_dir);
    fprintf('Max Area: %.4f m² (%.2f cm²)\n', max(A), max(A_cm2));
    fprintf('Max Stress Ratio: %.3f\n', max(stress_ratio));
    fprintf('Generated comprehensive plots for form finding analysis\n');
    fprintf('========================================\n');
...
%% FORM FINDING SPECIFIC PLOTTING FUNCTION
function generateFormFindingPlots(NODE, BARS, FIX_NODE, FREE_NODE, A_cm2, stress_ratio, stress_values, S_final, F_member, ...
                                 fixed_edge_indices, free_edge_indices, FontSizes, PE_detailed_FF, PE_FF_history, ...
                                 Nn, Nb, L0, A)
    ...
    %% 1. ENHANCED PE AND GRADIENT TRACKING FOR FORM FINDING
    if ~isempty(PE_detailed_FF) && ~isempty(PE_detailed_FF.values)
        figure('Name','Form Finding: PE and Gradient Convergence','Position',[50 50 1400 800]);
        
        % PE Linear Scale
        subplot(2,3,1);
        plot(PE_detailed_FF.iterations, PE_detailed_FF.values, 'b-o', 'LineWidth', 2, 'MarkerSize', 4);
        ...
        title('PE Convergence - Form Finding', 'FontSize', FontSizes.Title, 'FontName', FontSizes.FontName);
        set(gca, 'FontSize', FontSizes.TickLabel, 'FontName', FontSizes.FontName);
        grid on;
        
        % PE Log Scale
        subplot(2,3,2);
        semilogy(PE_detailed_FF.iterations, abs(PE_detailed_FF.values), 'b-s', 'LineWidth', 2, 'MarkerSize', 4);
        xlabel('fminunc Iteration', 'FontSize', FontSizes.AxisLabel, 'FontName', FontSizes.FontName);
        ylabel('|PE| (log scale)', 'FontSize', FontSizes.AxisLabel, 'FontName', FontSizes.FontName);
        title('PE Convergence (Log) - Form Finding', 'FontSize', FontSizes.Title, 'FontName', FontSizes.FontName);
        set(gca, 'FontSize', FontSizes.TickLabel, 'FontName', FontSizes.FontName);
        grid on;
        
        % Gradient Norm Convergence
        subplot(2,3,3);
        valid_grad_idx = ~isnan(PE_detailed_FF.gradient_norms);
        if sum(valid_grad_idx) > 0
            valid_iters = PE_detailed_FF.iterations(valid_grad_idx);
            valid_grads = PE_detailed_FF.gradient_norms(valid_grad_idx);
            semilogy(valid_iters, valid_grads, 'b-^', 'LineWidth', 2, 'MarkerSize', 5);
            xlabel('fminunc Iteration', 'FontSize', FontSizes.AxisLabel, 'FontName', FontSizes.FontName);
            ylabel('||∇PE|| (log scale)', 'FontSize', FontSizes.AxisLabel, 'FontName', FontSizes.FontName);
            title('Gradient Norm → 0', 'FontSize', FontSizes.Title, 'FontWeight', 'bold', 'FontName', FontSizes.FontName);
            set(gca, 'FontSize', FontSizes.TickLabel, 'FontName', FontSizes.FontName);
            grid on;
        else
            text(0.5, 0.5, 'Gradient data not available', 'Units', 'normalized', 'HorizontalAlignment', 'center', 'FontName', FontSizes.FontName);
            title('Gradient Norm (No Data)', 'FontSize', FontSizes.Title, 'FontName', FontSizes.FontName);
        end
        
        % Max Gradient Component
        subplot(2,3,4);
        valid_max_grad_idx = ~isnan(PE_detailed_FF.max_gradient_components);
        if sum(valid_max_grad_idx) > 0
            valid_iters_max = PE_detailed_FF.iterations(valid_max_grad_idx);
            valid_max_grads = PE_detailed_FF.max_gradient_components(valid_max_grad_idx);
            ...
            title('Max Gradient → 0', 'FontSize', FontSizes.Title, 'FontWeight', 'bold', 'FontName', FontSizes.FontName);
            set(gca, 'FontSize', FontSizes.TickLabel, 'FontName', FontSizes.FontName);
            grid on;
            ...
            title('Max Gradient (No Data)', 'FontSize', FontSizes.Title, 'FontName', FontSizes.FontName);
        end
        
        % PE Reduction Rate
        subplot(2,3,5);
        if length(PE_detailed_FF.values) > 1
            PE_diff = diff(PE_detailed_FF.values);
            plot(PE_detailed_FF.iterations(2:end), -PE_diff, 'b-v', 'LineWidth', 2, 'MarkerSize', 4);
            xlabel('fminunc Iteration', 'FontSize', FontSizes.AxisLabel, 'FontName', FontSizes.FontName);
            ylabel('PE Reduction per Step', 'FontSize', FontSizes.AxisLabel, 'FontName', FontSizes.FontName);
            title('Energy Reduction Rate', 'FontSize', FontSizes.Title, 'FontName', FontSizes.FontName);
            set(gca, 'FontSize', FontSizes.TickLabel, 'FontName', FontSizes.FontName);
            grid on;
        end
        ...
        axis off;
        stats_text = {};
        stats_text{end+1} = sprintf('FF Total Iterations: %d', length(PE_detailed_FF.values));
        stats_text{end+1} = sprintf('Initial PE: %.6e', PE_detailed_FF.values(1));
        stats_text{end+1} = sprintf('Final PE: %.6e', PE_detailed_FF.values(end));
        ...
        sgtitle('Form Finding: Complete Equilibrium Analysis', 'FontSize', 18, 'FontWeight', 'bold', 'FontName', FontSizes.FontName);
    end
    
    %% 2. PE HISTORY PLOT
    if ~isempty(PE_FF_history)
        figure('Name','Potential Energy History - Form Finding','Position',[100 100 800 600]);
        plot(1:length(PE_FF_history), abs(PE_FF_history), 'b-o', 'LineWidth', 2, 'MarkerSize', 5);
        xlabel('Form Finding Iteration', 'FontSize', FontSizes.AxisLabel, 'FontName', FontSizes.FontName);
        ylabel('|Potential Energy|', 'FontSize', FontSizes.AxisLabel, 'FontName', FontSizes.FontName);
        title('Potential Energy Evolution - Form Finding', 'FontSize', FontSizes.Title, 'FontName', FontSizes.FontName);
        set(gca, 'FontSize', FontSizes.TickLabel, 'FontName', FontSizes.FontName);
        grid on;
        
        final_pe = abs(PE_FF_history(end));
    ...
    %% 4. AREA PLOT (Given Areas)
    figure('Name','Member Areas (Given)','Position',[100 200 1000 700]);
    hold on; axis equal; grid on; box on;
    
    % Color mapping for areas
    max_area = max(A_cm2);
    min_area = min(A_cm2);
    area_norm = (A_cm2 - min_area) / (max_area - min_area + eps);
    cmap = parula(256);
    ...
    ylabel('Y (m)', 'FontSize', FontSizes.AxisLabel, 'FontWeight', 'bold', 'FontName', FontSizes.FontName);
    zlabel('Z (m)', 'FontSize', FontSizes.AxisLabel, 'FontWeight', 'bold', 'FontName', FontSizes.FontName);
    title('Grid Shell - Given Member Cross-Sectional Areas', 'FontSize', FontSizes.Title, 'FontWeight', 'bold', 'FontName', FontSizes.FontName);
    set(gca, 'FontSize', FontSizes.TickLabel, 'FontName', FontSizes.FontName);
    view(30,40);
    ...
    if area_range > eps
        ...
        mid_val = (min_area + max_area) / 2;
        caxis([mid_val-0.1, mid_val+0.1]);
    end
    cb = colorbar;
    ...
    %% 8. MEMBER CLASSIFICATION PLOT
    A_stats = A_cm2(A_cm2 > 1);
    if ~isempty(A_stats)
        ...
        n_small = sum(A_cm2 > 1 & A_cm2 <= med_threshold);
        n_very_small = sum(A_cm2 <= 1);
        
        pie_data = [n_small, n_med, n_large, n_very_small];
        pie_labels = {sprintf('Small\n(%d)', n_small), sprintf('Medium\n(%d)', n_med), ...
                      sprintf('Large\n(%d)', n_large), sprintf('Very Small\n(%d)', n_very_small)};
        ...
        stats_text = {
            ...
            sprintf('Large Areas (>%.1f cm²): %d', large_threshold, n_large);
            sprintf('Medium Areas: %d', n_med);
            sprintf('Small Areas: %d', n_small);
            sprintf('Very Small Areas (≤1 cm²): %d', n_very_small);
            sprintf('Max Area: %.2f cm²', max(A_cm2));
            sprintf('Mean Area: %.2f cm²', mean(A_cm2));
            sprintf('Total Volume: %.6f m³', sum(A .* L0));
        };
    ...
    %% 9. AXIAL FORCE PLOT (with colormap)
