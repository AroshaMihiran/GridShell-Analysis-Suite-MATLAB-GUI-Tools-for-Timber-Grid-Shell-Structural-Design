function [PANELS, TRIB_AREA, PANEL_AREAS, NODE_FORCES, F_nodal] = gridshell_panels_loads(NODE, BARS, q_panels, opts)
...
N = size(NODE,1);
TRIB_AREA   = zeros(N,1);
NODE_FORCES = zeros(N,1);     % vertical nodal forces
F_nodal     = zeros(3*N,1);   % global force vector
    ...
    case 'equal'
        for p = 1:numel(PANELS)
            ...
            A = PANEL_AREAS(p);
                ...
                % equal share
            A_share = A/k * ones(k,1);

                % compute nodal forces from panel pressure
            F_share = q_panels(p) * A_share;

                % accumulate
            TRIB_AREA(vids)   = TRIB_AREA(vids)   + A_share;
            NODE_FORCES(vids) = NODE_FORCES(vids) + F_share;

                % add to global vector (Z only)
            for ii = 1:k
                node = vids(ii);
                F_nodal(3*node) = F_nodal(3*node) - F_share(ii);
            end
        end
